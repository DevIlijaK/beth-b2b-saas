...
export const authController = new Elysia({
  ...
  .post(
    "/signInOrUp",
    async ({ body: { name, password, action }, auth, set, headers }) => {
      let user;
      ...
      if (action === "signup") {
        user = await auth
          .createUser({
            key: {
              providerId: "basic",
              providerUserId: name.trim().toLowerCase(),
              password,
            },
            attributes: {
              name,
        ...
        console.log("uslo gde treba");
        user = await auth.useKey("basic", name.toLowerCase(), password);
      } else {
